...
def _derive_host_output_dirs(
    ...
    if request_class not in {"isolated_runtime_request", "artifact_update", "artifact_regeneration"} and "task-runner" not in enabled_roles:
        return []
...
def resolve_from_request(
    ...
    if request_class == "productionization":
        enabled_roles = ["app"]
        enabled_modes = ["rebuild-image"] if capabilities.get("supports_rebuild_image", False) else _smallest_valid_modes(plan)
        ...
        enabled_modes = ["rebuild-image"] if "rebuild" in text and capabilities.get("supports_rebuild_image", False) else _smallest_valid_modes(plan)
    ...
    resolved_plan = {
        ...
        "image_resolution_policy": image_resolution_policy,
    }
