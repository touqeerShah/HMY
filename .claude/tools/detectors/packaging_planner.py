...
def collect_application_image(
    ...
    if runtime == "node":
        return (
            f"node:{version or '20'}-{variant}"
            if variant
            else f"node:{version or '20'}-slim"
        )
    if runtime == "python":
...
def _build_capabilities(rules: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "supports_host_output_dirs": bool(
...
def _build_suggested_artifacts(
    target: dict[str, Any], rules: dict[str, Any]
) -> list[str]:
    ...
    if rules.get("supports_command_runner", False):
        suggested.extend(
            [
    ...
    if rules.get("supports_task_runner", False) and rules.get(
        "supports_command_runner", False
    ):
        suggested.extend(
            [
    ...
    if rules.get("supports_host_output_dirs", False):
        suggested.extend(
            [
                ".docker-data/logs",
